...
float C74GenAudioProcessor::getParameter (int index)
{
    ...
    switch(index){
        case 0:
            thrustValue = normalisedValue;
            thrustLabelValue = String(int(value));
            break;
        case 1:
            mixValue = normalisedValue;
            mixLabelValue = String(value * 100) + " %";
            break;
        case 2:
            modeValue = normalisedValue;
...
void C74GenAudioProcessor::setParameter (int index, float newValue)
{
	...
	gen_exported::setparameter(m_C74PluginState, index, newValue, NULL);
...
void C74GenAudioProcessor::processBlock (AudioSampleBuffer& buffer, MidiBuffer& midiMessages)
{
    ...
    isNowPlaying = (this->getPlayHead()->getPosition()->getIsPlaying());
...
bool C74GenAudioProcessor::isCurrentlyPlaying()
{
    return isNowPlaying;
}
...
Value& C74GenAudioProcessor::getThrustValueObject()
    {
        return thrustValue;
    }
Value& C74GenAudioProcessor::getThrustLabelObject()
    {
        return thrustLabelValue;
    }
...
Value& C74GenAudioProcessor::getModeValueObject()
    {
        return modeValue;
    }
