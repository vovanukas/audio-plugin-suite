...
NewComponent::NewComponent ()
{
    ...
    auto& fb1Obj = feedback1->getValueObject();
    fb1Obj.referTo (processor.getFb1ValueObject());
    auto& fb1Lab = fb1Label->getTextValue();
    fb1Lab.referTo(processor.getFb1LabelObject());

    auto& fb2Obj = feedback2->getValueObject();
    fb2Obj.referTo (processor.getFb2ValueObject());
    auto& fb2Lab = fb2Label->getTextValue();
    fb2Lab.referTo(processor.getFb2LabelObject());

    auto& mixObj = mix->getValueObject();
    mixObj.referTo (processor.getMixValueObject());
    auto& mixLab = mixLabel->getTextValue();
    mixLab.referTo(processor.getMixLabelObject());

    auto& dampObj = damp->getValueObject();
    dampObj.referTo (processor.getDampValueObject());

    auto& spreadObj = spread->getValueObject();
    spreadObj.referTo (processor.getSpreadValueObject());
    //[/Constructor]
}
...
void NewComponent::sliderValueChanged (juce::Slider* sliderThatWasMoved)
{
    ...
    else if (sliderThatWasMoved == mix.get())
    {
        ...
        processor.setParameter(3, mix->getValue());
        //[/UserSliderCode_mix]
    }
